 ...
 */
@Data
@TableName("STORE_STRATEGY")
@EqualsAndHashCode(callSuper = false)
@Accessors(chain = true)
@ApiModel(value="STORE_STRATEGY对象", description="设备运行时数据存储策略")
public class StoreStrategy {
	...
	/**存储类型，如定点存储、定时存储*/
	@Excel(name = "存储类型，如定点存储、定时存储", width = 15)
    @ApiModelProperty(value = "存储类型，如定点存储、定时存储")
